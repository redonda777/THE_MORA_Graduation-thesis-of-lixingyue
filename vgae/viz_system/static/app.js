...
const parallelReader = document.getElementById('parallelReader');
const parallelReaderTitle = document.getElementById('parallelReaderTitle');
const parallelReaderMeta = document.getElementById('parallelReaderMeta');
const parallelReaderBody = document.getElementById('parallelReaderBody');
const parallelReaderClose = document.getElementById('parallelReaderClose');
...
function closeParallelReader() {
  state.parallelSentence = null;
  parallelReader.classList.add('hidden');
  parallelReader.setAttribute('aria-hidden', 'true');
  parallelReaderBody.innerHTML = '';
}

function setParallelHighlight(sentence, options = {}) {
  const sentenceKey = String(sentence);
  state.parallelSentence = Number(sentence);
  const segments = parallelReaderBody.querySelectorAll('.parallel-segment');
  segments.forEach(segment => {
    const active = segment.dataset.sentence === sentenceKey;
...
function parallelMeta(payload) {
  const nearest = payload.nearest || {};
  if (nearest.source === 'sentence' && nearest.distance !== null && nearest.distance !== undefined) {
    return `第 ${Number(payload.chapter) + 1} 章 · ${payload.focus.version} ↔ ${nearest.version} · normalized_distance ${formatScore(nearest.distance)}`;
  }
  return `第 ${Number(payload.chapter) + 1} 章 · ${payload.focus.version} ↔ ${nearest.version} · 章节相似度 ${formatScore(nearest.similarity)}`;
}

function renderParallelReading(payload) {
  const [left, right] = payload.columns || [];
  parallelReaderTitle.textContent = `第 ${Number(payload.chapter) + 1} 章对读`;
  parallelReaderMeta.textContent = parallelMeta(payload);
  parallelReaderBody.innerHTML = [left, right].map(column => `
    <section class="parallel-column ${column.side}">
      <h3>${escapeHtml(column.version)}</h3>
      <div class="parallel-lines">
        <div class="parallel-text-flow">
          ${column.rows.map(row => `<span class="parallel-segment" role="button" tabindex="0" data-sentence="${row.sentence}" data-node-id="${escapeHtml(row.nodeId)}" title="第 ${row.sentenceDisplay} 句">${escapeHtml(row.text)}</span>`).join('')}
        </div>
      </div>
    </section>
  `).join('');

  parallelReaderBody.querySelectorAll('.parallel-segment').forEach(segment => {
    segment.addEventListener('click', () => {
      setParallelHighlight(segment.dataset.sentence);
    });
  ...
  parallelReader.classList.remove('hidden');
  parallelReader.setAttribute('aria-hidden', 'false');
  setParallelHighlight(payload.focus.sentence);
}

async function openParallelReading(node) {
  if (!node || node.type !== 'sentence') return;
  parallelReader.classList.remove('hidden');
  parallelReader.setAttribute('aria-hidden', 'false');
  parallelReaderTitle.textContent = '章节对读';
  parallelReaderMeta.textContent = '正在抽取原文...';
  parallelReaderBody.innerHTML = '<div class="parallel-loading">正在读取 Mora.xlsx 原文</div>';
  try {
    const payload = await getJson(`/api/parallel-reading?chapter=${node.chapter}&version=${encodeURIComponent(node.version)}&sentence=${node.sentence}`);
    renderParallelReading(payload);
  } catch (err) {
    parallelReaderMeta.textContent = '';
    parallelReaderBody.innerHTML = `<div class="parallel-loading">${escapeHtml(err.message)}</div>`;
  }
}
...
function showSentenceDetail(node) {
  ...
  detailPanel.innerHTML = `
    <div class="detail-title sentence-detail-title">
      <span>${node.version} 第 ${sentenceDisplay(node.sentence)} 句</span>
      <button class="parallel-read-btn" id="parallelReadBtn" type="button">对读</button>
    </div>
  ...
  `;
  document.getElementById('parallelReadBtn')?.addEventListener('click', () => {
...
async function searchAndFocusSentence() {
  ...
  const result = await getJson(`/api/search-sentences?q=${encodeURIComponent(query)}&limit=12`);
...
parallelReaderClose.addEventListener('click', closeParallelReader);

parallelReader.addEventListener('click', event => {
  if (event.target === parallelReader) closeParallelReader();
});

window.addEventListener('keydown', event => {
  if (event.key === 'Escape' && !parallelReader.classList.contains('hidden')) {
    closeParallelReader();
  }
});
